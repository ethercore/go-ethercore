...
func TestDefaultGenesisBlock(t *testing.T) {
	block := DefaultEtherCoreGenesisBlock().ToBlock(nil)
	if block.Hash() != params.EtherCoreGenesisHash {
		t.Errorf("wrong mainnet genesis hash, got %v, want %v", block.Hash(), params.EtherCoreGenesisHash)
	}
...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantHash:   params.EtherCoreGenesisHash,
			wantConfig: params.EtherCoreChainConfig,
		},
		{
			name: "mainnet block in DB, genesis == nil",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				DefaultEtherCoreGenesisBlock().MustCommit(db)
				return SetupGenesisBlock(db, nil)
			},
			wantHash:   params.EtherCoreGenesisHash,
			wantConfig: params.EtherCoreChainConfig,
