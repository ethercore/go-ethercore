...
import (
	...
	"github.com/ethercore/go-ethercore/accounts/keystore"
	"github.com/ethercore/go-ethercore/common"
	"github.com/ethercore/go-ethercore/common/fdlimit"
	...
	"github.com/ethercore/go-ethercore/crypto"
	"github.com/ethercore/go-ethercore/eth"
	...
	"github.com/ethercore/go-ethercore/ethstats"
	"github.com/ethercore/go-ethercore/graphql"
	"github.com/ethercore/go-ethercore/les"
	"github.com/ethercore/go-ethercore/log"
	"github.com/ethercore/go-ethercore/metrics"
	"github.com/ethercore/go-ethercore/metrics/influxdb"
	"github.com/ethercore/go-ethercore/miner"
	"github.com/ethercore/go-ethercore/node"
	"github.com/ethercore/go-ethercore/p2p"
	"github.com/ethercore/go-ethercore/p2p/discv5"
	"github.com/ethercore/go-ethercore/p2p/enode"
	"github.com/ethercore/go-ethercore/p2p/nat"
	"github.com/ethercore/go-ethercore/p2p/netutil"
	"github.com/ethercore/go-ethercore/params"
	"github.com/ethercore/go-ethercore/rpc"
	whisper "github.com/ethercore/go-ethercore/whisper/whisperv6"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Network identifier (integer, 1=Frontier, 2=Morden (disused), 3=Ropsten, 4=Rinkeby, 466=Ethercore, 467=EtherCore PoA, 468=EtherCore Test)",
	...
	EtherCoreFlag = cli.BoolFlag{
		Name:  "ethercore",
		Usage: "EtherCore network: pre-configured ethercore network",
	}
	EtherCorePoAFlag = cli.BoolFlag{
		Name:  "poa",
		Usage: "EtherCore PoA network: pre-configured ethercore poa network",
	}
	EtherCoreTestFlag = cli.BoolFlag{
		Name:  "test",
		Usage: "EtherCore Test network: pre-configured ethercore test network",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		...
		if ctx.GlobalBool(EtherCoreFlag.Name) {
			return filepath.Join(path, "ethercore")
		}
		if ctx.GlobalBool(EtherCorePoAFlag.Name) {
			return filepath.Join(path, "poa")
		}
		if ctx.GlobalBool(EtherCoreTestFlag.Name) {
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	urls := params.EtherCoreBootnodes
	switch {
	...
	case ctx.GlobalBool(EtherCoreFlag.Name):
		urls = params.EtherCoreBootnodes
	case ctx.GlobalBool(EtherCorePoAFlag.Name):
		urls = params.EtherCorePoABootnodes
	case ctx.GlobalBool(EtherCoreTestFlag.Name):
		urls = params.EtherCoreTestBootnodes
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(EtherCoreFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "ethercore")
	case ctx.GlobalBool(EtherCorePoAFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "poa")
	case ctx.GlobalBool(EtherCoreTestFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "test")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	case ctx.GlobalBool(EtherCoreFlag.Name):
		...
		cfg.Genesis = core.DefaultEtherCoreGenesisBlock()
	case ctx.GlobalBool(EtherCorePoAFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 467
		}
		cfg.Genesis = core.DefaultEtherCorePoAGenesisBlock()
	case ctx.GlobalBool(EtherCoreTestFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 468
		}
		cfg.Genesis = core.DefaultEtherCoreTestGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
